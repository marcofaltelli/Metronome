...
unsigned long pkts_to_queue = 250;
unsigned long time1, time2;
int num_cores, custom_timer_mode;
bool adaptive, first_time = true;
...
double rho;
unsigned long vacation_period = VACATION_TIME, timeout_short;
...
static inline void my_sleep(long int x, unsigned long y, unsigned long z){
        asm volatile(
                        "mov %%rbx,%%rdi\n"
                        "mov %%rcx,%%rsi\n"
                        "syscall\n"
...
//		printf("Got lock!\n");
		if (!first_packet) {
                                delta =((float) pkts_idle) - pkts_idle_mean;
                                pkts_idle_mean += delta / ++count;
                                delta2 = ((float) pkts_idle) - pkts_idle_mean;
                                pkts_idle_var += delta * delta2;
                        }
		time2 = __rdtscp(&junk);
		...
		for (i = 0; i < qconf->nb_rx_queue; ++i) {
			...
			do { // poll as long as pkts available 
...
//			lock[i] = 0;
		...
		if (likely(!first_time)) {
                        //if (tot_nb_rx > 0) {
                                vacation_time = (time2 - time1)/2.1;
                                busy_time = (__rdtscp(&junk) - time2)/2.1;
                                lambda_local = ((float)(tot_nb_rx)) / ((float) (vacation_time + busy_time));
                                lambda = ALPHA * lambda + (1 - ALPHA) * lambda_local;
                                new_timer = (unsigned long) (((float) pkts_to_queue) / lambda);
				...
				if (!first_packet) {
		                        process_tries++;
                                        delta_vacation =  ((double) vacation_time - mean_vacation);
                                        mean_vacation += delta_vacation/((double) process_tries);
                                        delta2_vacation =  ((double) vacation_time - mean_vacation);
                                        var_vacation += delta_vacation * delta2_vacation;
                	                mean_busy += ((double) busy_time - mean_busy)/((double) process_tries);
                                }
                                timeout_local = timeout_short;
			//}
                //if (j++%2000 == 0)
		//	printf("vacation time %lu busy %lu lambda %f pkts seen %lu timeout is %lu\n", vacation_time, busy_time, rho, tot_nb_rx, timeout_short);
                //}
		}
		first_time = false;
...
#ifdef CUSTOM
			my_sleep(SYSCALL_ENTRY, timeout_local, 1);
#endif
#ifndef CUSTOM
...
main(int32_t argc, char **argv)
{
	...
	/*ret = rte_vdev_init("cryptodev_aesni_mb","socket_id=1,max_nb_queue_pairs=2");
	if (ret < 0)
		rte_exit(EXIT_FAILURE,"Can't load crypto device\n");
	*/
	/* init EAL */
